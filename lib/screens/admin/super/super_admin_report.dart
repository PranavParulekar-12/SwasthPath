...
class SuperAdminScheme extends StatefulWidget {
  const SuperAdminScheme({super.key});

  @override
  State<SuperAdminScheme> createState() => _SuperAdminSchemeState();
}

class _SuperAdminSchemeState extends State<SuperAdminScheme> {
  ...
  Future<Map<String, int>> fetchSchemeDataFromSupabase() async {
    // Fetch all schemes data for the last 8 days
  ...
  Future<List<Map<String, dynamic>>> fetchSchemeTrend(String scheme) async {
    // Fetch trend data for a specific scheme over the last 8 days
    final eightDaysAgo = DateTime.now().subtract(const Duration(days: 8));
    final response = await Supabase.instance.client
